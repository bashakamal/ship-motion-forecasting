...
if run_btn:
    ...
    if heave_col and heave_col != "accn_z":
        df_raw["accn_z"] = df_raw[heave_col]
    has_heave = heave_col is not None
    ...
    IMU_COLS = ["roll_deg", "pitch_deg", "yaw_deg", "gx", "gy", "gz", "accn_z"]
    t0, t1 = ts[0], ts[-1]
    ...
    df = df_rs.copy()
    outlier_log = {}
    clean_cols = ["roll_deg", "pitch_deg"] + (["accn_z"] if has_heave else [])
...
has_heave = d.get("has_heave", False)
heave_raw = (df_clean["accn_z"].values.astype(np.float32) if has_heave else None)
CTX_360 = int(360 * FS)
model = load_model()
...
with tab_pred:
    ...
    ctx_heave = heave_raw[cut - ctx:cut] if has_heave else None
    ...
    for sig_label, ctx_sig, pred_key, unit in sig_specs:
        ...
        for i, h_sec in enumerate(horizons):
            ...
            ax.set_ylabel(f"{sig_label} ({unit})")
    ...
    st.caption("For each horizon: the predicted peak and RMS of each motion, and "
               "whether it is expected to get rougher or calmer vs the matching slice "
               "of recent history. Roll/Pitch in degrees, Heave in m/s².")
    ...
    for h_sec in horizons:
        ...
        for sig_label, ctx_sig, pred_key, unit in sig_specs:
            ...
            q_rows.append({
                "Horizon": f"next {h_sec}s", "Signal": sig_label, "Unit": unit,
    ...
    for h_sec in horizons:
        r = results[h_sec]
        for j, (tp, pp, pr) in enumerate(zip(r["t_pred"], r["p_pitch"], r["p_roll"])):
            row = {"horizon_sec": h_sec,
...
with tab_stats:
    ...
    stat_signals = ["Roll", "Pitch"] + (["Heave"] if has_heave else [])
    rows = []
    ...
    unit_of = {"Roll": "deg", "Pitch": "deg", "Heave": "m/s²"}
    for sig in stat_signals:
        st.markdown(f"**{sig} motion**  ·  units: {unit_of[sig]}")
...
with tab_analytics:
    ...
    # ── Peak / RMS / H1/3 for each signal ─────────────────────────────────────
    st.divider()
    analytics_signals = [s for s in ["Roll", "Pitch", "Heave"]
                         if s in stat_df["Signal"].unique()]
    unit_lbl = {"Roll": "deg", "Pitch": "deg", "Heave": "m/s²"}
    st.markdown("### Predicted descriptors — " + " · ".join(analytics_signals))
    colors_met = {"Peak": "#D85A30", "RMS": "#185FA5", "H1/3": "#3B6D11"}
    fig, axes = plt.subplots(1, len(analytics_signals),
                             figsize=(8 * len(analytics_signals), 5), squeeze=False)
    axes = axes[0]
    for ax, sig in zip(axes, analytics_signals):
        ...
        ax.set_xlabel("Forecast horizon")
        ax.set_ylabel(f"Predicted amplitude ({unit_lbl[sig]})")
